...
const episodeData = {
 ...
 // async/await
test("renders season list when select season button is clicked", async () => {
    mockFetchShow.mockResolvedValueOnce(episodeData);
    const { getByText, getAllByText, debug, getByTestId } = render(<App />);

    await waitFor(()=> {const drop = getByText(/select a season/i)})
    const drop = getByText(/select a season/i)
    userEvent.click(drop);
