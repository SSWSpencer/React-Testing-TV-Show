...
test("Renders list of episodes after API call", () => {
    const { rerender, debug, getByText, getAllByTestId } = render(<Episodes episodes={[]} />);
    rerender(<Episodes episodes={episodeData} />)

    const chapOne = getByText(/chapter one:/i);
    const chapTwo = getByText(/chapter two:/i);
    const episodeList = getAllByTestId(/episode/i);

    expect(chapOne).toBeInTheDocument();
    expect(chapTwo).toBeInTheDocument();
